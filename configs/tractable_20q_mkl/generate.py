...
SIGMA_CFG = {
    "type": "mkl",
    "mkl_weights": [1/6, 1/6, 1/6, 1/6, 1/6, 1/6],
    "mkl_base_kernels": ["density", "nn_h", "nn_v", "nn_d", "plaquette", "global_anchor"],
    "grid_shape": GRID,
    "sigma": SIGMAS,
}
...
for dset_key, param_list in DATASETS.items():
    for param_suffix, params in param_list:
        for topo_key, topo_cfg in TOPOLOGIES.items():
            ...
            doc = {
                "output": {"base_dir": "out_tractable_20q_mkl", "suite_name": name},
                "runs": [run],
            }
