"""Scale-free network Ising samples dataset loaded from PennyLane."""
...
class ScaleFreeDataset(BinaryDataset):
    """
    Scale-free network Ising model samples for generative quantum ML.

    Loads the ``scale-free`` dataset from PennyLane (20 000 training samples
    of 1 000 spins from an Ising model defined on a Barabasi-Albert
    scale-free graph with random couplings, sampled via MCMC).  Spin
    values are converted from +/-1 to {0, 1} and then PCA-reduced to
    ``rows x cols`` qubits.
    ...
    def generate(self, n_samples: int, seed: int = 0) -> np.ndarray:
        """
        Sample *n_samples* PCA-reduced scale-free network vectors.
    ...
    def visualize(self, sample: np.ndarray, ax=None):
        """
        Visualize a single scale-free sample as a 2-D binary grid.
        """
        if ax is None:
            fig, ax = plt.subplots(figsize=(2, 2))

        sample_2d = sample.reshape((self.rows, self.cols))
        ax.imshow(sample_2d, cmap="Oranges", aspect="equal", interpolation="nearest")
        ax.set_title("Scale-Free sample")
