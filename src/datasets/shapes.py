...
class BinaryShapesDataset(BinaryDataset):
    ...
    def is_valid(self, sample: np.ndarray) -> bool:
        """
        Check if a sample is valid (contains at least one shape pixel).
        Since all samples generated by this class are valid by construction,
        we check if the sample has any non-zero elements.
        """
        return np.any(sample)

    def validity_rate(self, samples: np.ndarray) -> float:
        """
        Compute fraction of samples that are valid (contain at least one pixel).
    ...
    def coverage_rate(self, ground_truth: np.ndarray, samples: np.ndarray) -> float:
        """
        Compute coverage rate as the fraction of unique ground truth samples
        that appear in the generated samples.
        
        Args:
            ground_truth (np.ndarray): Ground truth samples of shape (n_gt, n_qubits)
            samples (np.ndarray): Generated samples of shape (n_samples, n_qubits)
            
        Returns:
            float: Fraction of ground truth samples found in generated samples
        """
        if len(ground_truth) == 0 or len(samples) == 0:
            return 0.0
        
        # Convert to tuples for set operations
        ground_truth_set = set(map(tuple, ground_truth))
        samples_set = set(map(tuple, samples))
        
        # Count overlap
        overlap = len(ground_truth_set & samples_set)
