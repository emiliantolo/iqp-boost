"""Binarized Fashion-MNIST dataset integration."""
...
class FashionMNISTDataset(BinaryDataset):
    """Fashion-MNIST images resized to a binary grid and flattened to bitstrings."""
    ...
    def __init__(
        ...
        if classes is not None:
            ...
            if invalid:
                raise ValueError(f"Fashion-MNIST classes must be labels 0-9, got {invalid}")
            if not classes:
    ...
    def generate(self, n_samples: int | None = None, seed: int = 0, split: str = "train") -> np.ndarray:
        ...
        if split == "all":
            raise ValueError("FashionMNISTDataset supports split='train' or split='test'")
    ...
    def generate_balanced(self, n_per_class: int, seed: int = 0, split: str = "train") -> np.ndarray:
        ...
        for label in classes:
            ...
            if len(class_indices) == 0:
                raise ValueError(f"No Fashion-MNIST samples found for class {label}")
    ...
    def _load_split_with_labels(self, split: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        dataset = FashionMNIST(root=str(self.data_dir), train=(split == "train"), download=True)
        images = dataset.data
        ...
        if self.classes is not None:
            ...
            if len(images) == 0:
                raise ValueError(f"No Fashion-MNIST samples found for classes {self.classes}")
